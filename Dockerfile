...
COPY package.json tsconfig.json tslint.json $APP_ROOT
...
\
# Cleaning up
&& rm -f $APP_ROOT/mecab-${MECAB_VERSION}.tar.gz \
&& rm -rf $APP_ROOT/mecab-${MECAB_VERSION} \
&& rm -f $APP_ROOT/mecab-ipadic-${IPADIC_VERSION}.tar.gz \
&& rm -rf $APP_ROOT/mecab-ipadic-${IPADIC_VERSION} \
&& rm -rf $APP_ROOT/mecab-ipadic-neologd \
\
